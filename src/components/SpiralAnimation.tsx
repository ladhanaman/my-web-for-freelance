...
class AnimationController {
  ...
  private readonly changeEventTime = 0.42
  private readonly cameraZ = -400
  private readonly cameraTravelDistance = 3400
  private readonly startDotYOffset = 28
  private readonly viewZoom = 60
  ...
  private drawTrail(t1: number) {
    for (let i = 0; i < this.trailLength; i++) {
      const f = this.map(i, 0, this.trailLength, 0.65, 0.1)
      const sw =
      ...
      const pathTime = t1 - 0.00028 * i
      const position = this.spiralPath(pathTime)

      const basePos = position
      const offset = new Vector2D(position.x + 12, position.y + 12)
      const rotated = this.rotate(
        basePos,
        offset,
        Math.sin(this.time * Math.PI * 2 + i * 0.15) * 0.5 + 0.5,
        i % 2 === 0
      )
...
class Star {
  ...
  constructor(cameraZ: number, cameraTravelDistance: number) {
    ...
    this.distance = 39.68 * Math.random() + 19.84  // +15% from prev (34.5/17.25)
    this.rotationDirection = Math.random() > 0.5 ? 1 : -1
    this.expansionRate = 2.3 + Math.random() * 1.38  // +15% from prev (2.0/1.2)
    this.finalScale = 0.805 + Math.random() * 0.69  // +15% from prev (0.7/0.6)

    this.dx = this.distance * Math.cos(this.angle)
    this.dy = this.distance * Math.sin(this.angle)

    this.spiralLocation = (1 - Math.pow(1 - Math.random(), 1.0)) / 1.3
    this.z = Vector2D.random(0.5 * cameraZ, cameraTravelDistance + cameraZ)

    const lerp = (start: number, end: number, t: number) =>
      start * (1 - t) + end * t
    this.z = lerp(this.z, cameraTravelDistance / 2, 0.3 * this.spiralLocation)
    this.strokeWeightFactor = Math.pow(Math.random(), 1.0)  // pow 1 = even size distribution
  }
  ...
  render(p: number, controller: AnimationController) {
    ...
    if (q > 0) {
      ...
      const dotSize = 28 * this.strokeWeightFactor * sizeMultiplier
...
export function SpiralAnimation() {
  ...
  // Handle resize — initialise from actual window on client
  useEffect(() => {
    const handleResize = () => {
      setDimensions({
